...
export default async function LegacyDetailRedirect(props: PageProps<"/catalog/materials/[id]">) {
  const { id } = await props.params;
  const item = await db.catalogItem.findUnique({ where: { legacyMaterialId: id }, select: { id: true } });
  if (!item) notFound();
