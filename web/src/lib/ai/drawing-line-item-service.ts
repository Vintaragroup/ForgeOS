...
An elevated second-level platform or mezzanine deck -- a raised deck on support posts, with a guardrail around its edge, connected to the floor by its own staircase -- is a real, substantial structure that deserves its own named entry, even on a whole-booth overview/isometric sheet that otherwise gets one flat entry for its own title. Give it its own entry ONLY when a raised deck, a guardrail, and a connecting staircase are all genuinely, visually identifiable in the rendering -- don't go looking for a platform that isn't there just because this instruction exists; a booth shown with a single flat, ground-level floor gets no such entry. Name it using this document's own corner/zone naming convention when it's determinable from the page itself or from a floor-plan/legend sheet elsewhere in the document (e.g. one titled "CALLOUTS") -- e.g. "Elevated Platform - Left Back Corner" -- falling back to a plain descriptive name like "Elevated Platform" only when no corner/zone name is determinable anywhere in the document. Two visually distinct platforms (e.g. one in each back corner) are two separate entries with two different names, never merged into one.
...
A single elevation can have MORE THAN ONE such overall dimension, not just one -- e.g. a full-width total spanning end-to-end (including end caps or returns) AND a second, narrower total spanning just the main repeating section inside it, both printed on the same elevation. The same is true for height: a full-height total and one or more shorter sub-totals for just part of that same wall (a base/kick, a main field, a cap) can all appear on one elevation. Every one of these overall/sub-total numbers is a check-dimension, no matter how many there are -- exclude all of them as items, and never let one of these totals get attributed to a panel on a DIFFERENT elevation elsewhere on the sheet just because the numbers happen to look similar.

Read each individual dimension value from its own witness/extension lines only -- never blend, prepend, or append a digit from a different, nearby label onto a segment's real value. A narrow end-cap or return segment sitting close to an overall/aggregate total on the same dimension line is the case this matters most for: a real segment printed as "19.53"" sitting near an unrelated "224.59"" or "290.50"" total elsewhere on the sheet must never be reported as "219.53"" or any other value that merges a stray digit in from that nearby total just because they happen to be near each other or start with the same digit. Before reporting a value, confirm you can point to one single, complete, standalone numeral at one specific pair of arrows/tick marks -- if what you're about to report doesn't match any one number actually printed that way, re-read the isolated segment instead of combining nearby text.
...
When a sheet shows more than one elevation, fully work through EACH one's own individual segment dimensions and propose items for every one of them -- don't stop after the first elevation you process, and don't let a denser or more visually prominent elevation crowd out a smaller one elsewhere on the same sheet. A sheet with two elevations showing 6 segments on one and 5 on the other has 11 real segments to account for in total, not just whichever set you noticed first.
...
An elevated second-level platform/mezzanine deck -- a raised deck on support posts, with a guardrail around its edge and its own connecting staircase -- is a genuinely different case from the floor-plan/legend page above, even though both can lack a printed dimension: that page is a bare 2D zone-location index with nothing of its own to price, while a platform like this is a real, physically built (or rented) structure you can actually see, fully assembled, on a 3D isometric/overview sheet -- propose it as its own line item even when no dimension line anywhere in the document gives it a printed numeric size. Only propose it when a raised deck, a guardrail, and a connecting staircase are all genuinely, visually confirmable in the rendering -- this is not an instruction to go looking for a platform on every booth; most booths have none. One item per visually distinct platform (e.g. one per back corner of a two-corner booth) -- each platform you can actually see and count is its own item, qty 1 EA, qtyIsExplicit: true, the same directly-countable logic already covered above for a "Left & Right"-titled element. category is "Flooring & Platforms". Describe honestly and only what's actually visually confirmable -- a raised deck on support posts, a guardrail, a connecting staircase, and the stair's tread count only when it's clearly countable in the rendering -- and when the platform's overall size is inferred or approximate rather than read from a printed value, say so directly in the description itself (e.g. "approximate footprint -- no dimension printed on this sheet") -- the same honesty rule already applied to height above applies here to a platform's overall size. Never invent a precise-looking width x height the sheet never actually gives; an honestly approximate description is correct here, a fabricated-looking exact number is not.
...
export async function proposeLineItemsFromDrawing(
  ...
  // Real gap this closes (Sept 2026): a real production run took 6+
  // minutes across 6 AI calls (~5-7x slower than the same file measured
  // locally against direct OpenAI), and there was no way to tell from
  // that alone whether production is routing through a different,
  // slower path (AI_DRAWING_MODEL set to an OpenRouter model) or just
  // experiencing real network/infra latency direct-to-OpenAI calls don't
  // hit locally -- this makes that visible in server logs on the next run.
  console.log(`[proposeLineItemsFromDrawing] document ${documentId}: using model="${model}" viaOpenRouter=${viaOpenRouter}.`);
